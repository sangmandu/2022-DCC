...
def get_dataloader(batch_size):
    ...
    train_dataset = Dataset(train_df, resize=RESIZE, type='train')
    eval_dataset = Dataset(eval_df, resize=RESIZE, type='eval')

    dataloaders = {}
    dataloaders['train'] = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True,
                                                       num_workers=1)
    dataloaders['val'] = torch.utils.data.DataLoader(eval_dataset, batch_size=batch_size, shuffle=False, num_workers=1)
    return dataloaders
...
class Encoder(nn.Module):
    def __init__(self, hidden_dim, num_layers, num_pos):
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=hidden_dim, nhead=32, dim_feedforward=640, dropout=0.1,
                                                        activation='gelu')
...
def train_model(model, encoder, optimizer, dataloaders, args):
    ...
    if warmup_step > 0:
        warmup_scheduler = optim.lr_scheduler.OneCycleLR(optimizer, max_lr=args.lr, total_steps=warmup_step * 2,
                                                         pct_start=0.5, anneal_strategy='linear')
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--val-cycle', default=5, type=int, help='perform validation every ? epochs')
